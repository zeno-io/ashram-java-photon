 ...
 * @author zeno
 */
public class T0642_DesignSearchAutocompleteSystem_2 {
...
// 👍 59 👎 0
  ...
  // 执行用时：227 ms, 在所有 Java 提交中击败了50.67% 的用户
  ...
  class AutocompleteSystem {
    ...
    public List<String> input(char c) {
      ...
      input.append(c);
      return search(input.toString(), N_RANK);
    }
    ...
    // 搜索以 sentence 为前缀的所有词句，返回热度前k个
    private List<String> search(String sentence, int k) {
      TrieNode curr = root;
      for (int i = 0; i < sentence.length(); i++) {
        char c = sentence.charAt(i);
        curr = get(curr, c);
      ...
      PriorityQueue<TrieNode> l = new PriorityQueue<>(this::compare0);
      if (curr.end > 0) {
        if (curr.pass == 1) {
          return Collections.singletonList(curr.sentence);
        }
      }
      recur(curr, l, k);
    ...
    // 返回前k个最热的词句，使用最小堆
    private void recur(TrieNode node, PriorityQueue<TrieNode> ans, int k) {
      if (node == null) {
        return;
      }
      if (node.end > 0) {
        if (ans.size() >= k) {
          if (compare0(ans.peek(), node) < 0) {
            ans.poll();
            ans.offer(node);
          }
        } else {
      ...
      for (int i = 0; i < node.next.length; i++) {
        recur(node.next[i], ans, k);
      }
    }

    // return negative , means o1 < o2
    private int compare0(TrieNode o1, TrieNode o2) {
      // 热度从高到低排序（第一个是最热门的）   最小堆 -> 热度从低到高排序
      if (o1.hotTime != o2.hotTime) {
        return o1.hotTime - o2.hotTime;
      }
